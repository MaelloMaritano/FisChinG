...
Game::Game(float width, float height):
	shaders(Shaders("Tappa07/shader.vert", "Tappa07/shader.frag")), camera(Camera(glm::vec3(0.0f, 0.4f, -2.4f), glm::vec3(5.0f, 0.0f, 0.0f), width, height)), scene(Scene(shaders, camera))
{
	srand(time(0));
	loadScene();
}
